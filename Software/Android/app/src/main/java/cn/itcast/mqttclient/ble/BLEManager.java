...
import cn.itcast.mqttclient.BLEDevice;
import cn.itcast.mqttclient.util.ClsUtils;
...
//        bluetoothGattService = bleManager.getBluetoothGattService(bluetoothGatt,ConsData.MY_BLUETOOTH4_UUID);
        ...
        for (BluetoothGattCharacteristic characteristic : bluetoothGattService.getCharacteristics()) {
            Log.e(TAG, "character : "+ characteristic.getUuid().toString());
    ...
    public boolean sendCmd(byte[] cmd)
    {
        ...
        boolean  b = writeCharacteristic.setValue(cmd);
        Log.d(TAG, "写特征设置值结果：" + b);
        return mBluetoothGatt.writeCharacteristic(writeCharacteristic);
    }
